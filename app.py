...
def GET():
    ...
    for i, k in enumerate(results.values):
        ...
        mosaic = im[y:y+h, x:x+w]   # 관심영역 지정
        mosaic = cv2.resize(mosaic, (w//rate, h//rate)) # 축소
        mosaic = cv2.resize(mosaic, (w,h), interpolation=cv2.INTER_AREA) # 확대
        im[y:y+h, x:x+w] = mosaic   # 원본 이미지에 적용
